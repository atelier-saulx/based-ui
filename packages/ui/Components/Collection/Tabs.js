...
const Tab = ({ tab, onClick, activeTab, index, tabSizes }) => {
...
export const Tabs = ({
  ...
  useEffect(() => {
    global.requestAnimationFrame(() => {
      ...
      const tabSizes = []
      const xTop = ref.current.getBoundingClientRect().x
  ...
  return (
    <div
      ...
      style={{
        ...
        paddingBottom: noBorder ? null : 20,
        borderBottom: noBorder ? null : '1px solid ' + useColor('outline'),
        ...style
      }}
    >
      {tabs.map((tab, index) => {
